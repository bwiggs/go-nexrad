...
func appendToFile(filename string, stuffToAppend string) {
	// If the file doesn't exist, create it, or append to the file
    f, err := os.OpenFile(filename, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
    if err != nil {
        log.Fatal(err)
    }
    if _, err := f.Write([]byte(stuffToAppend)); err != nil {
        log.Fatal(err)
    }
...
func render(out string, radials []*archive2.Message31, label string) {
	...
	svgHeader := "<?xml version=\"1.0\"?><!-- Generated by SVGo --><svg width=\"" + strconv.Itoa(int(width)) + "\" height=\"" + strconv.Itoa(int(height)) + "\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">"

	PNGcanvas := image.NewRGBA(image.Rect(0, 0, int(width), int(height)))
	draw.Draw(PNGcanvas, PNGcanvas.Bounds(), image.Transparent, image.ZP, draw.Src)
